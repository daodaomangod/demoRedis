...
import com.swc.demo.config.KafkaProperties;
...
class SdevKafkaResourceIT {
    ...
    void setup() {
        ...
        SdevKafkaResource kafkaResource = new SdevKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/sdev-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/sdev-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
